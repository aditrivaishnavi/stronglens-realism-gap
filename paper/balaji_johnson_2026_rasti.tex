...
\\
$^{1}$BASIS Independent Silicon Valley, 1290 Parkmoor Ave, San Jose, CA 95126, USA\\
...
\renewcommand{\today}{21 June 2026}
...
% ====================================================================
\begin{abstract}
Calibrating the selection function of CNN strong lens finders relies on injection-recovery, yet to our knowledge the fidelity of parametric source models in CNN feature space has not been directly tested.  We train an EfficientNetV2-S classifier on $\sim\!450\,000$ cutouts from the DESI Legacy Imaging Survey DR10 and measure the gap between its internal representations of spectroscopically confirmed lenses and parametric S\'{e}rsic injections.  The model achieves $89.3$~per cent recall on 112 confirmed lenses, but standard injection-recovery yields only $\sim\!5$~per cent marginal completeness over the full parameter space and $29$--$39$~per cent for bright, favourably lensed injections, a factor of $2$--$3$ below real-lens recall.  A linear probe on the CNN's penultimate features separates real lenses from brightness-matched injections with AUC $= 0.997 \pm 0.003$, consistent with a morphological mismatch between parametric and real arc morphology (though host-population differences may also contribute).  We test whether this gap arises from missing noise texture by adding Poisson shot noise to injected arcs.  The effect is regime-dependent: Poisson noise \emph{reduces} marginal completeness but \emph{increases} detection of moderately bright arcs by up to $+10.5$ percentage points, consistent with added texture aiding marginally detectable arcs while degraded spatial coherence harms prominent ones.  These results demonstrate that arc-level shot noise alone does not close the sim-to-real gap, consistent with morphological realism being the limiting factor for parametric injection-recovery.  We provide completeness maps as lower bounds under the parametric model and propose linear-probe AUC as a practical realism gate for injection pipelines.
\end{abstract}

\begin{keywords}
gravitational lensing: strong -- surveys -- techniques: image processing -- Data Methods -- Machine Learning
\end{keywords}
...
The advent of convolutional neural network (CNN) lens finders has transformed strong lens discovery.  Modern CNNs achieve high recall on confirmed lenses \citep[e.g.][]{Petrillo2017,Lanusse2018,Jacobs2019,Canameras2021,Storfer2024} and have produced large candidate catalogues from wide-area surveys \citep[e.g.][]{Metcalf2019}.  However, the question of how to calibrate their selection functions remains open.  The standard approach uses parametric source models, typically S\'{e}rsic profiles lensed by singular isothermal ellipsoids (SIE) or singular isothermal spheres (SIS) with external shear, to generate synthetic lensed arcs for injection \citep[e.g.][]{CollettCunnington2022,Herle2024}.  This approach assumes that parametric models capture the morphological features that the CNN uses for detection.
...
\item A completeness map with fully quantified uncertainties for the specific parametric injection family, together with a diagnostic framework (linear-probe AUC) to assess and iteratively improve injection realism.
\end{enumerate}
...
The gain of $150\;\mathrm{e^{-}\;nmgy^{-1}}$ is an order-of-magnitude estimate for a typical DR10 $r$-band coadd of $\sim\!30$ exposures at $90\;\mathrm{s}$ each.  We do not claim this is exact; instead, we use a gain sweep experiment (Section~\ref{sec:gainsweep}) to demonstrate that the result is physical and not an artefact of gain miscalibration.
...
\begin{tabular}{lcr}
\toprule
$\theta_{\rm E}$ (arcsec) & $C(p>0.3)$ & $n_{\rm det}$ \\
\midrule
0.50 & 0.22\% & 22 \\
0.75 & 0.33\% & 33 \\
1.00 & 1.53\% & 153 \\
...
\textit{Alt text:} Two-panel line plot. The left panel shows injection-recovery completeness on a logarithmic vertical axis rising from near zero at small Einstein radius to a peak of about eight per cent near 2.5~arcsec, then declining at larger radii. The right panel shows completeness versus lensed apparent magnitude in four bins, highest at bright magnitudes and falling toward faint magnitudes, with the no-Poisson and Poisson curves lying close together.
\end{figure*}
...
\label{fig:umap}
\textit{Alt text:} Two-panel UMAP scatter plot of CNN penultimate-layer embeddings. The left panel is coloured by category and shows real Tier-A lenses occupying a region largely separated from low- and high-$\beta_{\rm frac}$ injections and from negatives. The right panel shows the same points coloured by CNN score on a scale from zero to one, with the real-lens region scoring high and the injection and negative regions scoring low.
\end{figure*}
...
\label{fig:scores}
\textit{Alt text:} Histogram with a logarithmic vertical axis showing CNN score distributions. Validation negatives are concentrated near zero, brightness-matched injections are spread across low to moderate scores, and real Tier-A lenses are concentrated near one. Vertical lines mark the detection thresholds at scores of 0.3, 0.806, and 0.995.
\end{figure}
...
\label{fig:brightarc}
\textit{Alt text:} Line plot of detection rate versus source apparent magnitude for six experimental conditions. All curves peak at intermediate magnitudes and decline toward faint magnitudes. A horizontal dashed line marks the 89.3 per cent Tier-A real-lens recall, which lies well above every injection curve.
\end{figure}
...
To confirm that the Poisson effect is physical and not a code artefact, we repeated the bright-arc experiment at gain~$= 10^{12}\;\mathrm{e^{-}\;nmgy^{-1}}$, where Poisson noise is negligible ($\sigma \sim 3 \times 10^{-6}\;\mathrm{nmgy\;pixel^{-1}}$).  Detection rates match the no-Poisson baseline at every magnitude bin to within $0.5$~pp ($= 1/N$), confirming three things: (i) the Poisson code path is correct (it adds zero effective noise when the gain is very high), (ii) at gain~$= 150$, the Poisson noise is physically large enough to change detection rates, and (iii) the regime-dependent effects seen in Table~\ref{tab:brightarc} are not artefacts of gain miscalibration.
...
The gain sweep confirms the physical nature of both effects.  A distributional component may also contribute (the model was not trained on Poisson-noised arcs), but the sign reversal at small $\theta_{\rm E}$ argues against a purely distributional explanation: if Poisson noise were simply out-of-distribution, it should degrade detection uniformly rather than improving it in specific regimes.
...
Four limitations bear directly on interpreting the completeness estimates; three further caveats are then noted.
...
Second, our Poisson noise test targets a specific class of texture mismatch, namely missing arc shot noise, because the host and background already come from real DR10 cutouts.  We do not claim to have ruled out all texture mismatches.  In particular, we do not model correlated noise in the coadd imaging.  Real coadds have spatially correlated noise from the dithering and resampling process, which has a fundamentally different spatial structure from independent shot noise.  While independent Poisson noise disrupts spatial coherence pixel-by-pixel, correlated noise creates spatially coherent fluctuations that could in principle make injections look more like real survey features rather than less.  The effect of correlated coadd noise on injection detectability is therefore an open question that cannot be inferred from the independent-noise result.
...
We propose the linear-probe AUC as a quantitative realism gate for the community to evaluate and iteratively improve injection pipelines.  Applied to DESI Legacy Imaging Survey DR10 with an EfficientNetV2-S lens finder, our analysis yields the following results.

\begin{enumerate}
\item[(i)] The classifier achieves $\sim\!89$~per cent recall (95~per cent CI: $[83, 94]$~per cent) on over 100 spectroscopically confirmed Tier-A lenses, with zero spatial overlap between training and validation sets.

\item[(ii)] Standard injection-recovery with parametric S\'{e}rsic sources yields only $\sim\!5$~per cent marginal completeness over the full parameter space, and $\sim\!12$~per cent even at the brightest lensed magnitudes ($20$--$22$), consistent with a morphological mismatch between parametric injections and real lensed galaxies.

\item[(iii)] A linear probe in the CNN's penultimate feature space separates real lenses from brightness-matched injections with AUC $= 0.997 \pm 0.003$, establishing that the CNN distinguishes them based on properties beyond brightness alone.  The Fr\'{e}chet distance grows $330\times$ from the earliest to mid-level network layers (0.14 to 47.2), consistent with the separation emerging in learned features rather than low-level pixel statistics.

\item[(iv)] Adding arc-level Poisson noise has a regime-dependent effect: it reduces marginal completeness but increases detection of moderately bright arcs by up to $+10.5$ percentage points.  This dual behaviour, with added texture aiding marginally detectable arcs and degraded spatial coherence harming prominent ones, is confirmed by a gain-sweep control and paired per-injection analysis.

\item[(v)] Under our simplified PSF model, the injection realism gap is consistent with a \emph{morphological barrier}: parametric S\'{e}rsic profiles lack the spatially coherent substructure of real lensed galaxies.  The injection completeness map should be interpreted as a lower bound conditional on the parametric prior; a different source model could yield materially different completeness.
\end{enumerate}

Replacing parametric sources with real galaxy stamps \citep[e.g.\ from HUDF, as in][]{Canameras2024} and re-evaluating the linear-probe AUC would test whether the morphological barrier can be closed.  Extending the analysis to other architectures (e.g.\ Vision Transformers) and forthcoming surveys (Euclid, LSST) would establish whether the gap is architecture- or survey-dependent.
...
National Laboratory (LBNL).  Legacy Surveys also use data products from
the Near-Earth Object Wide-field Infrared Survey Explorer (NEOWISE), a
project of the Jet Propulsion Laboratory/California Institute of
Technology, funded by the National Aeronautics and Space Administration.
Legacy Surveys was supported by: the Director, Office of Science, Office
of High Energy Physics of the U.S.\ Department of Energy; the National
Energy Research Scientific Computing Center, a DOE Office of Science
User Facility; the U.S.\ National Science Foundation, Division of
...
California under contract to the U.S.\ Department of Energy.  The
complete acknowledgements can be found at
\url{https://www.legacysurvey.org/acknowledgment/}.

AI language models (Claude, Anthropic; ChatGPT, OpenAI) were used as
coding assistants during the development of analysis scripts and for
manuscript copy-editing.  All AI-assisted code was reviewed, tested, and
verified by the authors, who are solely responsible for the scientific
content of this paper.

This work made use of the following open-source scientific software:
\textsc{PyTorch} and \textsc{torchvision} for model training and inference;
\textsc{NumPy}, \textsc{SciPy}, \textsc{pandas}, and \textsc{pyarrow} for
numerical and tabular data handling; \textsc{scikit-learn} for the linear
probe and classification metrics; \textsc{healpy} for HEALPix spatial
splits; \textsc{astropy} for coordinate transforms; \textsc{Matplotlib}
for figures; \textsc{UMAP-learn} for the embedding visualisation; and
\textsc{lenstronomy} for independent cross-validation of the SIE
deflection in the test suite.

\section*{Data availability}
The injection pipeline code, selection function grid results (per-cell completeness tables), bright-arc test results, CNN model weights, and the linear probe analysis scripts are publicly available at \url{https://github.com/aditrivaishnavi/stronglens-realism-gap} and will be archived on Zenodo with a DOI assigned at publication.  The training manifest, Tier-A and Tier-B lens catalogues, and per-injection metadata are included in the repository.  The DESI Legacy Imaging Survey DR10 data are publicly available at \url{https://www.legacysurvey.org/dr10/}.
...
These 12 lenses represent the $10.7$~per cent ($12/112$; 95~per cent Wilson CI: $[5.6, 18.1]$~per cent) failure rate of the CNN on confirmed lenses.  Visual inspection suggests the failures share common morphological characteristics: edge-on deflector galaxies, compact double-image configurations, or lensed features that are faint relative to the deflector light.  A detailed morphological classification is beyond the scope of this paper.

\begin{table}
\centering
\caption{The 12 Tier-A lenses missed by the CNN at the $p > 0.3$ detection threshold.  Sorted by ascending CNN score.  $z_{\rm lens}$ from the DESI Strong Lensing catalogue cross-matched by position ($< 12\;\mathrm{arcmin}$ from the brick centre; the large radius reflects the offset between the Legacy Survey brick centre and the lens position within the $\sim\!0.25\;\mathrm{deg}$ brick, not a positional uncertainty; each association is unique, with at most one catalogue entry per brick).  All missed lenses have deflector $r$-band magnitudes $\leq 20$.}
\label{tab:missed}
\small
\setlength{\tabcolsep}{4pt}
\begin{tabular}{llccl}
