...
\\
$^{1}$BASIS Independent Silicon Valley, 1290 Parkmoor Ave, San Jose, California\\
$^{2}$Steward Observatory, The University of Arizona, 933 North Cherry Avenue, Tucson, Arizona
}

\date{Accepted XXX. Received YYY; in original form ZZZ}
\pubyear{2026}

\renewcommand{\today}{25 May 2026}
...
% ====================================================================
\begin{abstract}
Calibrating the selection function of CNN strong lens finders relies on injection-recovery, yet to our knowledge the fidelity of parametric source models in CNN feature space has not been directly tested.  We train an EfficientNetV2-S classifier on $\sim\!450\,000$ cutouts from the DESI Legacy Imaging Survey DR10 and measure the gap between its internal representations of spectroscopically confirmed lenses and parametric S\'{e}rsic injections.

The model achieves $89.3$~per cent recall on 112 confirmed lenses, but standard injection-recovery yields only $\sim\!5$~per cent marginal completeness over the full parameter space and $29$--$39$~per cent for bright, favourably lensed injections, a factor of $2$--$3$ below real-lens recall.  A linear probe on the CNN's penultimate features separates real lenses from brightness-matched injections with AUC $= 0.997 \pm 0.003$, consistent with a morphological mismatch between parametric and real arc morphology (though host-population differences may also contribute).

We test whether this gap arises from missing noise texture by adding Poisson shot noise to injected arcs.  The effect is regime-dependent: Poisson noise \emph{reduces} marginal completeness but \emph{increases} detection of moderately bright arcs by up to $+10.5$ percentage points, consistent with added texture aiding marginally detectable arcs while degraded spatial coherence harms prominent ones.

These results demonstrate that arc-level shot noise alone does not close the sim-to-real gap, consistent with morphological realism being the limiting factor for parametric injection-recovery.  We provide completeness maps as lower bounds under the parametric model and propose linear-probe AUC as a practical realism gate for injection pipelines.
\end{abstract}
...
The injection pipeline code, selection function grid results (per-cell completeness tables), bright-arc test results, CNN model weights, and the linear probe analysis scripts will be archived on Zenodo (DOI to be assigned on acceptance) and are available at \url{https://github.com/aditrivaishnavi/stronglens-realism-gap} (to be made public on acceptance).  The training manifest, Tier-A and Tier-B lens catalogues, and per-injection metadata will be included in the archive.  The DESI Legacy Imaging Survey DR10 data are publicly available at \url{https://www.legacysurvey.org/dr10/}.
