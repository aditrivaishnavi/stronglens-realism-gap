...
Four limitations bear directly on interpreting the completeness estimates; we list these first, followed by three additional caveats.
...
\item[(iv)] Adding arc-level Poisson noise has a regime-dependent effect: it reduces marginal completeness but increases detection of moderately bright arcs by up to $+10.5$ percentage points.  This dual behaviour --- added texture aiding marginally detectable arcs, degraded spatial coherence harming prominent ones --- is confirmed by a gain-sweep control and paired per-injection analysis.
...
Two natural extensions follow.  First, replacing parametric sources with real galaxy stamps \citep[e.g.\ from HUDF, as in][]{Canameras2024} and re-evaluating the linear-probe AUC would test whether the morphological barrier can be closed.  Second, extending the analysis to other architectures (e.g.\ Vision Transformers) and forthcoming surveys (Euclid, LSST) would establish whether the gap is architecture- or survey-dependent.
